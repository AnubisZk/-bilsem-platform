...
import {
  Brain, Lock, BookOpen, CheckCircle, Clock,
  MessageSquare, Star, TrendingUp, ChevronDown,
  LogOut, Target, Calendar, Award,
} from 'lucide-react';
...
export default function StudentPortalPage() {
  ...
  async function handleLogin(e: any) {
    ...
    setLoading(true);
    try {
      const portalData = await api.get(`/students/${id}/portal-data`);
      const student = portalData.data;
      ...
      <div className="min-h-screen bg-gradient-to-br from-brand-50 via-white to-violet-50 dark:from-gray-950 dark:via-gray-900 dark:to-brand-950 flex items-center justify-center p-4">
        ...
        >
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold font-display text-gray-900 dark:text-white">BİLSEM Öğrenci Portalı</h1>
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f3] dark:bg-[#161614]">
      {/* Header */}
      <header className="bg-white dark:bg-[#1e1e1c] border-b border-black/[0.06] dark:border-white/[0.06] sticky top-0 z-10">
        <div className="max-w-4xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <button
            onClick={() => { setIsLoggedIn(false); setStudentData(null); setPassword(''); }}
            className="btn-ghost py-1.5 px-3 text-sm text-red-400"
          >
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 space-y-5">
        ...
        >
          ...
          {/* Mini istatistikler */}
          <div className="grid grid-cols-3 gap-3 mt-5 pt-5 border-t border-black/[0.04] dark:border-white/[0.04]">
        ...
        {plan ? (
          ...
          >
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-semibold text-gray-900 dark:text-white">Çalışma Planım</h3>
              <span className="badge badge-brand text-xs ml-auto">{plan.title}</span>
            </div>

            <div className="space-y-2">
              {weeks.map((week: any) => {
                const weekItems = planItems.filter((item: any) => item.week === week);
                ...
                return (
                  <div key={week} className="border border-black/[0.06] dark:border-white/[0.06] rounded-xl overflow-hidden">
                    ...
                    >
                      ...
                      <span className="font-medium text-sm text-gray-900 dark:text-white">{week}. Hafta</span>
                              ...
                              <div key={item.id} className="px-4 py-3">
                                <div className="flex items-start gap-3">
                                  ...
                                  <div className="flex-1">
                                    ...
                                    {item.activity && (
                                      <div className="mt-2 p-2 bg-brand-50 dark:bg-brand-900/20 rounded-lg">
              ...
              <div key={log.id} className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
                ...
                <div className="flex items-center gap-0.5 flex-shrink-0">
                  {Array.from({ length: 5 }, (_, i) => (
                    <Star key={i} className={cn('w-3 h-3', i < log.performance ? 'text-amber-400 fill-amber-400' : 'text-gray-200 dark:text-gray-700')} />
                  ))}
                </div>
              </div>
            ))}
          ...
          >
            ...
            <div className="space-y-3">
              {studentData.feedbacks.map((fb: any) => (
                <div key={fb.id} className="p-4 bg-gradient-to-r from-brand-50 to-violet-50 dark:from-brand-900/20 dark:to-violet-900/20 rounded-xl">
                  <p className="text-xs font-semibold text-brand-600 dark:text-brand-400 mb-2">{fb.period}</p>
                  <div className="space-y-2">
                    <div>
                      <p className="text-[10px] font-semibold uppercase tracking-wide text-emerald-600 mb-1">Güçlü Yönlerin</p>
                      <p className="text-xs text-gray-700 dark:text-gray-300 leading-relaxed">{fb.strengths}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-semibold uppercase tracking-wide text-amber-600 mb-1">Gelişim Alanların</p>
                      <p className="text-xs text-gray-700 dark:text-gray-300 leading-relaxed">{fb.improvements}</p>
                    </div>
                    {fb.motivation && (
                      <div className="mt-2 p-2 bg-white/50 dark:bg-white/5 rounded-lg">
