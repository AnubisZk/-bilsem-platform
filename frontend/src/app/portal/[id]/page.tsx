...
export default function StudentPortalPage() {
          ...
          <div className="card p-6">
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <button type="submit" disabled={loading} className="btn-primary w-full justify-center py-3">
                {loading ? <span className="flex items-center gap-2"><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Giriş yapılıyor...</span> : 'Giriş Yap'}
              </button>
            </form>
          </div>
  ...
  planItems.forEach((item: any) => weekSet.add(item.week));
    ...
    <div className="min-h-screen bg-[#f5f5f3]">
      <header className="bg-white border-b border-black/[0.06] sticky top-0 z-10">
        <div className="max-w-4xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <button onClick={() => { setIsLoggedIn(false); setStudentData(null); setPassword(''); }} className="btn-ghost py-1.5 px-3 text-sm text-red-400">
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 space-y-5">
              ...
              {weeks.map((week: number) => {
                ...
                return (
                  <div key={week} className="border border-black/[0.06] rounded-xl overflow-hidden">
                    <button onClick={() => setExpandedWeek(isExpanded ? null : week)} className="w-full flex items-center gap-3 px-4 py-3 bg-gray-50 hover:bg-gray-100 transition-colors">
                                  ...
                                  <div className="flex-1">
                                    ...
                                    {item.activity && (
                                      <div className="mt-2 p-2 bg-brand-50 rounded-lg">
                                        <p className="text-xs text-brand-600 flex items-center gap-1"><BookOpen className="w-3 h-3" />{item.activity.title}</p>
                                      </div>
                                    )}
                                  </div>
                                  <span className="text-xs text-gray-400 flex-shrink-0 flex items-center gap-1"><Clock className="w-3 h-3" />{item.duration} dk</span>
                                </div>
                              </div>
                            ))}
                          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="card p-5">
          ...
          <div className="space-y-2">
            ...
            {(studentData.studentLogs || []).length === 0 && <p className="text-sm text-gray-400 text-center py-6">Henüz etkinlik kaydın yok</p>}
          </div>
        </motion.div>
